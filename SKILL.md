...
**Diversified presentation (Phase T Stage 6, default off — `GAOTTT_EXPLORE_DIVERSIFIED_PRESENTATION_ENABLED=1` to enable):** with the flag on, `diversity > 0` additionally re-selects the natural slots at the engine layer via MMR — a `top_k × explore_diversity_pool_multiplier` candidate slice, an `explore_min_semantic` raw-cosine floor for laterals, and a cohort penalty so one file/cohort cluster cannot fill the list. Forced (`tag_filter`/`persona_context`) items keep their legacy ordering. `diversity=0.0` / `None` is bit-for-bit legacy even with the flag on.

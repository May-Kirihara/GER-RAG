...
- ✅ MV4: control plane（Postgres）— 2026-07-03 完了（独立パッケージ `control/` + asyncpg + FastAPI + 番号付き plain SQL migration（bootstrap-aware、1 file = 1 txn、alembic 不使用）/ 7 domain テーブル + default tenant bootstrap INSERT / host token 認証（平文は一度だけ発行、SHA-256 hash を DB 保存）/ audit log 同一トランザクション（J12、audit INSERT 失敗 = mutation rollback）/ supervisor 連携（3-point gate で ControlClient 構築、lifespan start/stop、`/route`・create/delete で arecord_event、`GET /admin/status` で auth failure state expose）/ degraded mode（network error で local 継続 + spool 蓄積 → 復旧で再送）/ permanent auth failure（401 で POST 停止 + spool 書き込み継続）/ usage spool idempotency（batch_id UUID4 + temp/fsync/atomic rename + asyncio.Lock）/ 9 integration scenarios + unit test 一式 / **J1=A: usage は route_resolution telemetry で operation count ではない、billing-grade は MV4.1**（PM 承認済み SoT deviation）。engine 非汚染（`gaottt/` 内で `import asyncpg` ゼロ、J9）。docs: [Operations — Control Plane](../wiki/Operations-Control-Plane.md) 新規 + Tuning knob 7 つ + Sidebar/Home + Architecture 設計判断表 3 行 + 本マーク）
- ✅ MV5: backup / DR（Litestream + runbook + DR drill）— 2026-07-04 完了（per-universe `gaottt.db` + `manifest.json` の 2 点セットが復旧対象、FAISS は決定論再構築で対象外・大宇宙向け optional。`scripts/gen_litestream_config.py` 純粋関数（`gaottt/multiverse/backup.py`）+ `deploy/litestream.yml` 雛形 + supervisor の `litestream_config_path` knob で create/delete 成功経路から best-effort・`_backup_hook_lock` で scan+write 直列化・atomic write・spawn env 隔離。`scripts/dr_drill.py` は standalone 宇宙で backup→破壊→restore→FAISS rebuild→起動時診断 green→top-1 determinism を検証、litestream binary 非依存 default。Codex review 3 巡: B1 stdout/stderr 分離 / B2 atomic write + stale write fence / B3 standalone 限定。新 knob 1 つ（`litestream_config_path`、default 空 = inert、`backup_gen_timeout_seconds` は不設置）。docs: [Operations — Backup & DR](../wiki/Operations-Backup-Multiverse.md) 新規 + Sidebar/Home/Tuning/Compact&Backup/Architecture + 本マーク）
- ✅ MV3.5: multiverse importer（standalone → multiverse 取り込み）— 2026-07-04 完了（MV3 follow-on。`gaottt/multiverse/importer.py`（純粋関数 + `execute_import` transaction scope）+ `scripts/import_universe.py`（CLI、exit code 0/2/3/4/5/6/7/8/9）。copy/move/dry-run の 3 mode。allowlist 方式で 7 file（`gaottt.db` / `-shm` / `-wal` / `*.faiss` / `*.faiss.ids` / `*.virtual.faiss` / `*.virtual.faiss.ids`）のみ copy、backup 系は skipped。post-copy `_verify_target_db` で file 存在 / `PRAGMA integrity_check == "ok"` / `sqlite_master` user table count ≥ 1 の 3 段階検証（0-byte / schemaless file を reject）。transaction scope `(allocate_port → mkdir → copy/move → integrity_check → manifest → create_universe)` を max 3 回 retry で wrap、例外時は copy mode で target cleanup、move mode は data loss 防止で cleanup しない。`--force` は source backend 停止確認のみ迂回（WAL size / integrity_check は迂回しない）。WP-1 test 66 + @slow 1 全緑。parity 対象外（`/reset` と同じ例外クラス、MCP/REST 非露出、CLI のみ）。docs: [Operations — Multiverse Import Universe](../wiki/Operations-Multiverse-Import-Universe.md) 新規 + Sidebar/Home/Troubleshooting/Multiverse-Setup/Architecture + 本マーク。仕様 SoT は [multiverse-importer-execution-plan.md](multiverse-importer-execution-plan.md) v3、handover は [handover-2026-07-04-multiverse-importer.md](handover-2026-07-04-multiverse-importer.md)）

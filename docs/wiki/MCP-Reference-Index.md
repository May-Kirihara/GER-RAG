...
| `recall` | 重力波伝播による検索 (Phase T: breakdown に `qualified`/`direct_score`/`field_score`/`lensing_gap` 追加 — **Phase U で qualification が default ON 昇格** + selection trace `c=`/`src=`/`learn=`、raw-top rescue) | [Memory](MCP-Reference-Memory.md) |
| `get_node` | ID 指定で 1 件取得 (read-only、seed pool 迂回) | [Memory](MCP-Reference-Memory.md) |
| `ambient_recall` | 構造化 passive recall（フックが毎ターン注入。Phase T: BM25 veto → semantic OR gate + `gate_diagnostics`/`empty_reason`。Phase U: composite gate mode 実装 (default は "or") + composite 軸 diagnostics） | [Memory](MCP-Reference-Memory.md) |
| `explore` | 高温の創発的探索 (Phase T: engine 層 MMR presentation — **Phase U WP-5 で default ON 昇格** + `wave: depth=N reached=M` header / selection trace) | [Memory](MCP-Reference-Memory.md) |

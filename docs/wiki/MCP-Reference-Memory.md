...
**Direct relevance qualification (Phase T Stage 3、default OFF・env opt-in `GAOTTT_DIRECT_QUALIFICATION_ENABLED=1`):** ON のとき direct 候補の presentation 選択が **qualified-first** になる — forced items (従来の RRF/raw 順序を維持) → qualified natural items (`final_score` 順) → 不足分のみ **fallback** (unqualified、`final_score` 順)。qualification は score 値のみで判断 (source 分岐ゼロ)、3 軸の OR: `raw_cos ≥ direct_raw_cosine_min` (決定論的軸) / `virtual_cos_norm ≥ direct_virtual_cosine_min` (正規化軸、新規計算) / BM25 lexical strength (score ≥ `direct_bm25_absolute_min` **AND** top score 比 ≥ `direct_bm25_relative_min`)。fallback pick は breakdown `qualified=false` + reason 行 `gravity pick (below relevance floor)` で明示され、TTT 更新も gate される (Stage 4 併用時)。MCP formatter は breakdown 行末尾に `q=+/q=- d=.. f=.. gap=..` を追記するだけで既存 segment は不変。閾値の較正根拠は golden corpus baseline (raw p50=0.764 の狭帯に対し 0.75 で 70.6% qualified) — [docs/notes/phase-t/](../notes/phase-t/) および [Plans — Phase T](Plans-Phase-T-Semantic-Requalification.md)。

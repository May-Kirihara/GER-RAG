...
| `ambient_gate_mode` | `"or"` | `GAOTTT_AMBIENT_GATE_MODE` | **WP-3 + §10 R3 follow-up — ambient gate mode**。`"or"` (default, Phase T Stage 5 の OR gate を bit-for-bit 維持) \| `"composite"` (3-arm: `accept = bm25_strong OR (virt_top1 ≥ ambient_composite_virt_hi) OR (bm25_top ≥ ambient_composite_bm25_mid AND virt_top1 ≥ ambient_composite_virt_mid)`)。**composite は 2 度の較正 (v2 単一 arm・v3 3-arm) とも事前登録昇格 gate (held-out FP=0 ∧ FN≤10%) を満たさず default は `"or"` のまま確定 (R3 close)** — 判定記録は [docs/notes/phase-u/ambient-composite-calibration.md](../notes/phase-u/ambient-composite-calibration.md)。opt-in 実験機構として残存 |
| `ambient_composite_virt_hi` | `0.85` (**未検証** provisional) | `GAOTTT_AMBIENT_COMPOSITE_VIRT_HI` | composite mode arm2 — top-1 virtual cosine の絶対閾値。v3 較正で昇格 gate 不達のため検証済み推奨値は存在しない |
| `ambient_composite_bm25_mid` | `22.0` (**未検証** provisional) | `GAOTTT_AMBIENT_COMPOSITE_BM25_MID` | composite mode arm3 の bm25 条件 — word-BM25 top score の中間閾値 |
| `ambient_composite_virt_mid` | `0.845` (**未検証** provisional) | `GAOTTT_AMBIENT_COMPOSITE_VIRT_MID` | composite mode arm3 の virt 条件 — arm2 より緩い中間閾値 (bm25 との AND 用) |

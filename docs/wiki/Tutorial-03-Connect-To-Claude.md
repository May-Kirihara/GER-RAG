# 🌱 はじめての GER-RAG (3/6) — Claude に接続

GER-RAG はパソコンに入りました。次は Claude Desktop に「GER-RAG を使ってね」と教えます。
...
`mcpServers` の中に `ger-rag-memory` の項目だけを追加してください（カンマの位置に注意）。
...
```json
{
  "mcpServers": {
    "ger-rag-memory": {
      "command": "C:\\Users\\taro\\GER-RAG\\.venv\\Scripts\\python.exe",
      "args": ["-m", "ger_rag.server.mcp_server"],
      "cwd": "C:\\Users\\taro\\GER-RAG"
    }
  }
}
```
...
クリックすると `ger-rag-memory` という名前と、たくさんの機能（remember, recall, ...）が並んで見えます。
...
```
GER-RAG memory が使えるか確認したいので、reflect ツールを aspect="summary" で呼んでみてもらえる？
```

### こう見えたら成功 ✅

Claude が `mcp__ger-rag-memory__reflect` ツールを呼んで、こんな感じの結果を返します:
...
- `ger-rag-memory` は MCP の名前。Claude はこの名前で機能を呼びます
- 接続後、Claude は GER-RAG が提供する **25 種類のツール**（remember, recall, explore...）を使えるようになります
